...
class BogotaLotesNormativaDict(Base):
    ...
    # Dictionary data
    variable = Column(
        String(21), nullable=True, comment="Variable name for normative mapping"
    )
    indice = Column(Integer, nullable=True, comment="Index value for the variable")
    input = Column(
        String(155),
        nullable=True,
        comment="Input value or description for the variable",
    )

    # Metadata
    fecha_update = Column(
        String(10), nullable=True, comment="Last update date of dictionary entry"
    )
